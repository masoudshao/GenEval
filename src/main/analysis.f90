...
program analysis
  ...
  integer, dimension(:), allocatable :: levels, prelevel
  ...
  elseif ((nran == 1).and.(nfix > 1)) then
     ...
     do i = 1, nobs
        ! -1 in next line because nfix was increment by one before to 
        ! acommodate mean value
        read(phenFileID, *) id(i), Xtemp(i, 1:(nfix-1)), y(i)
        k = 0
        do j = 1, (nfix-1)
          ! ignoring level=1 assuming levels start from 1
           if (xtemp(i,j) .eq. 1) cycle
           X(i, XTemp(i,j) + prelevel(j)) = ONE
        end do
     end do
     nfix = sum(levels)
     X(1:nobs, 1) = ONE ! finally a one to include the mean
  elseif (nran == 3) then
     call alloc2D(X, nobs, nfix, "X", "analysis")
     do i = 1, nobs
